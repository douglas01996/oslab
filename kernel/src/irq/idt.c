...
void init_idt() {
	...
	set_trap(idt+0x81,SEG_KERNEL_CODE,(uint32_t)vecsys2,DPL_USER);
	/* 设置外部中断的处理 */
...
//	set_intr(idt + 35, SEG_KERNEL_CODE, (uint32_t)irq2, DPL_KERNEL);
//	set_intr(idt + 37, SEG_KERNEL_CODE, (uint32_t)irq2, DPL_KERNEL);
	/* 写入IDT */
