...
CFLAGS := -Wall -Werror -Wfatal-errors -Wno-unused#开启所有警告, 视警告为错误, 第一个错误结束编译
CFLAGS += -MD #生成依赖文件
CFLAGS += -std=gnu11 -m32 -c -static#编译标准, 目标架构, 只编译
CFLAGS += -I ./include #头文件搜索目录
CFLAGS += -O0 #不开优化, 方便调试
CFLAGS += -fno-builtin -fno-stack-protector -fno-omit-frame-pointer#禁止内置函数
...
LD_SCRIPT2 := $(shell find $(GAME_DIR) -name "*.ld")
...
#	@cat $(BOOT) $(KERNEL) $(GAME)>$(IMAGE)
	...
	./format2 512 $(GAME) ctrlc hello test.txt test.dat douglas/test.dat douglas/douglas2/douglas3/test3.dat douglas/douglas2/test2.dat douglas/douglas2/douglas4/test.txt
	@$(DD) if=newdisk of=$(IMAGE) seek=201 conv=notrunc > /dev/null # 填充 kernel, 跨过 mbr
...
$(GAME): $(LD_SCRIPT2)
$(GAME): $(GAME_O) $(LIB_O)
#	$(LD) -m elf_i386 -e main_loop -nostdlib -o $@ $^ $(shell $(CC) $(CFLAGS) -print-libgcc-file-name)
	$(LD) -m elf_i386 -T $(LD_SCRIPT2) -nostdlib -o $@ $^ $(shell $(CC) $(CFLAGS) -print-libgcc-file-name)
	@game/fill_game.pl $@
